<section class="content-header">
    <h1>
        ...
        <small><?php echo __('View').' '.__('Student'); ?></small>
    </h1>
    <?php  $this->Html->addCrumb(__(' Home'), '/',['i','class'=>'fa fa-dashboard']);
            $this->Html->addCrumb(__('Students'),'/Students');
              $this->Html->addCrumb(__('View Student'));
...
<div class="panel panel-default" style="margin: 20px">
    <div class="panel-heading" align="right">
        <section class="content-header">
        	...
        	<a title='<?=__('Edit')?>' data-original-title='<?=__('Edit')?>' data-toggle='tooltip' href="<?php echo $this->Html->url(array('action' => 'edit', $student['Student']['id'])); ?>"><button class="btn btn-primary" type="button"><i class="fa fa-edit fa-fw"></i></button></a>
        	<?php echo $this->Form->postLink('<button title='.__('Delete').' data-original-title='.__('Delete').' data-toggle="tooltip" class="btn btn-primary"><i class="fa fa-trash-o fa-fw"></i></button>',
                                                ...
                                                array('escape'=>false),
                                                __('Are you sure you want to delete # %s?',  $student['Student']['id'])
                                            );?>
        </section>
    </div>
    <div class="nav-tabs-custom">
        <ul class="nav nav-tabs pull-right">
          <li class="active"><a data-toggle="tab" href="#tab_1-1" aria-expanded="false"><?=__("Student")?></a></li>
		...
		<?php if($student['Student']['nm_student']){?>
		<dt><?php echo __('Nm Student'); ?></dt>
		<dd><?php echo h($student['Student']['nm_student']); ?></dd>
		<?php }?>
                </div>
            </div>
                ...
                <div class="related">
                    ...
                    <?php if (!empty($student['Matriculation'])){ ?>
                            ...
                            <div style="margin: 20px">
                                <div id="dataTables-example_wrapper" class="dataTables_wrapper form-inline table-responsive" role="grid">                 
                                    <table  class="table table-striped table-bordered table-hover" id="dataTables-example">
                                        ...
                                        <tbody>
                                        	<?php foreach ($student['Matriculation'] as $matriculation): ?>
									<tr>
