...
<section class="content-header">
    ...
    <?php echo "<?php  \$this->Html->addCrumb(__(' Home'), '/',['i','class'=>'fa fa-dashboard']);\n";
          echo "            \$this->Html->addCrumb(__('{$pluralHumanName}'));\n";
...
<div class="<?php echo $pluralVar; ?> index" style="margin: 20px" >
    <div class="panel panel-default">
        <div class="panel-heading" align="right">
            <section class="content-header">
                <?php echo "<?php echo \$this->Html->link(\$this->Html->tag('button', \$this->Html->tag('i', '', array('class' => 'fa fa-refresh fa-fw')), array('class' => 'btn btn-primary','data-toggle'=>'tooltip','data-original-title'=>__('Refresh'),'title'=>__('Refresh'),'escape' => false)), \$this->Html->url(), array('escape' => false)); ?>\n"; ?>
                <?php echo "<?php echo \$this->Html->link(\$this->Html->tag('button', \$this->Html->tag('i', '', array('class' => 'fa fa-plus fa-fw')), array('class' => 'btn btn-primary','data-toggle'=>'tooltip','data-original-title'=>__('Add'),'title'=>__('Add'),'escape' => false)), \$this->Html->url(array('action' => 'add')), array('escape' => false)); ?>\n"; ?>
            </section>
        </div>
                	...
                	<thead>
                	   <tr>
                    	<?php foreach ($fields as $field): 
                    	    if($field == 'modified' ||$field == 'created' || $field == 'removed' || $field == 'id' || $field == 'date_cad'){
                    	        
                    	    }elseif($field == 'active'){?>
<th class="actions col-xs-1"><?php echo "<?php echo \$this->Paginator->sort('{$field}',__('{$field}'),array('data-toggle'=>'tooltip','data-original-title'=>__('{$field}'),'title'=>__('{$field}'))); ?>"; ?></th>
                            <?php
                            }else{ ?>
<th><?php echo "<?php echo \$this->Paginator->sort('{$field}',__('{$field}'),array('data-toggle'=>'tooltip','data-original-title'=>__('{$field}'),'title'=>__('{$field}'))); ?>"; ?></th>
                            <?php }
                                ...
                                if (!empty($associations['belongsTo'])) {
                                    foreach ($associations['belongsTo'] as $alias => $details) {
                                        if ($field === $details['foreignKey']) {
                                            $isKey = true;
                                            ?>
                                            <td><div data-original-title='<?="<?=__('{$field}')?>"?>' data-toggle="tooltip"><?php echo "<?php echo \$this->Form->input('{$field}', array('label' => '','default' => '','type'=>'select','class'=>'form-control input-sm selectpicker','data-style'=>'btn-primary','data-live-search'=>'true','style'=>'width:100%','empty' => __('{$field}')));?>"; ?></div></td>
                                            <?php
                                            break;
                                        }
                                    }
...
<?php echo "<?php \$options = array('S' => __('Active'),'N' => __('Inactive'));?>\n"?>
                            <td class="actions col-xs-1"><div data-original-title='<?="<?=__('{$field}')?>"?>' data-toggle="tooltip"><?php echo "<?php echo \$this->Form->input('{$field}', array('label' => '','default' => '','style'=>'width:100%','empty' => __('Status'),'class'=>'form-control input-sm selectpicker','data-style'=>'btn-primary','options' => \$options));?>"; ?></div></td>
                            <?php
                            }elseif($isKey){
                                
                            }else{ 

                    echo "\t\t\t\t\t <td><?php echo \$this->Html->div('row',\$this->Form->input('{$field}',array('div'=>'col-xs-12','style'=>'width:100%','label'=>'','data-toggle'=>'tooltip','data-original-title'=>__('{$field}'),'title'=>__('{$field}'),'class'=>'form-control input-sm')),array('escape'=>false));?></td>\n";
                    
                         }
                            
                         endforeach; ?>
<td><?php echo "<?php echo \$this->Form->submit(__('Search'),array('class' => 'btn btn-primary','data-toggle'=>'tooltip','data-original-title'=>__('Search'),'title'=>__('Search'))) ?>"?></td>
            		...
            		echo "\t\t\t\t\t\t\t<td class=\"actions\"  title='".__('Actions')."' data-original-title='".__('Actions')."' data-toggle=\"tooltip\">\n";
